...
export function QuizCreator() {
  ...
  const handleSave = async (sourceQuiz?: Quiz) => {
    setIsSaving(true);
    try {
      // When editing step-by-step, build quiz directly from stepQuestions
      let quizToSave = sourceQuiz || quiz;
      if (!quizToSave && editId) {
        const validQuestions = stepQuestions.filter(q =>
          q.text?.trim() && q.options?.every(opt => opt.trim()) && q.answer
        ) as Question[];
        if (validQuestions.length === 0) {
          alert("Please fill in at least one complete question.");
          return;
      ...
      const savedQuizzes = JSON.parse(localStorage.getItem("quizzes") || "{}");
      savedQuizzes[quizToSave.id] = quizToSave;
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold">Preview: {quiz.questions.length} Questions Found</h3>
              ...
              >
                <Save className="w-4 h-4" />
                {isSaving ? "Saving..." : quizKey ? "Update Key" : "Save & Get Key"}
              </button>
          </div>
